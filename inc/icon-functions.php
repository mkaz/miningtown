 ...
 */
function varia_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		$svg = varia_get_social_link_svg( $item->url, 26 );
		if ( empty( $svg ) ) {
			$svg = varia_get_icon_svg( 'link' );
		}
...
add_filter( 'walker_nav_menu_start_el', 'varia_nav_menu_social_icons', 10, 4 );
